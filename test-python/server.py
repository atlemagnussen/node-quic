...
class QuicTransportProtocol(QuicConnectionProtocol):
    ...
    def quic_event_received(self, event: QuicEvent) -> None:
        print("quic_event_received: start")
        try:
            ...
            print("quic_event_received error: {0}".format(e))
            self.handler = None
            self.close()
    ...
    def process_client_indication(self) -> None:
        print("process_client_indication: start")
        KEY_ORIGIN = 0
        KEY_PATH = 1
        indication = dict(
        ...
        print(f"origin was received {origin.hostname}")

        path = urllib.parse.urlparse(indication[KEY_PATH]).decode()

        print(f"path was received {path.path}")
